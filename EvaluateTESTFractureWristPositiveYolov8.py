...
f='best - 2epoch.pt'  # from utils.utils import *; strip_optimizer() 
#Strip optimizer from *.pt files for lighter files (reduced by 2/3 size) 
x = torch.load(f, map_location=torch.device('cpu')) 
x['optimizer'] = None 
torch.save(x, f)
"""
#torch.save(model.state_dict(), 'stripped_model1.pt') 
...
for i in range (len(imagesComplete)):
            ...
            #xmin,ymin,xmax,ymax=unconvert(imgTrue.shape[0],imgTrue.shape[1],xTrue,yTrue,xMaxTrue,yMaxTrue)
            colorTrue=(0,0,255)
            ...
            #imgTrue = cv2.rectangle(imgTrue, (xmin,ymin), (xmax,ymax),(0,255,0), 2)
            # Put text
